Public Structure SerialSend
    ...
    Public Function GetSerialData() As Byte()
        ...
        res(0) = If(Reset, 253, If(wheelPower < 0, 254, 255)) ' checkdigit + wheelPowerDir
        res(2) = Math.Abs(wheelPower)
        res(3) = windPower
        res(4) = shakeSpeed
        res(5) = shakePower
        res(1) = res(2) Xor res(3) Xor res(4) Xor res(5)
        'res(2) = leftPower + 128
...
Public Class SerialRead
    ...
    Public Sub SetSerialData(pSerialData As List(Of Byte))
        ...
        pedalAccel = pSerialData(4) + pSerialData(5) * 256
        pedalBreak = pSerialData(6) + pSerialData(7) * 256
        pedalClutch = pSerialData(8) + pSerialData(9) * 256
