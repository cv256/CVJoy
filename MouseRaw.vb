...
Public Class MouseRaw
    ...
    Public Enum DeviceUsage As UInt32
        Mouse = 0
        Keyboard = 1
        HID = 2
        Joy = 4
    ...
    ''' </summary>
    Public Shared Function GetMouses() As List(Of GetMousesResult)
        Dim result As New List(Of GetMousesResult)
        ...
        For i As UInt32 = 0 To count - 1
            ...
            If device.dwType <> DeviceUsage.Mouse AndAlso device.dwType <> DeviceUsage.Joy AndAlso device.dwType <> DeviceUsage.HID Then Continue For
            Dim deviceName As String = New String(" "c, 200) ', deviceInfo As String = New String(" "c, 200)
            GetRawInputDeviceInfo(device.hDevice, DeviceInfoTypes.RIDI_DEVICENAME, deviceName, deviceName.Length) ' windows internal deviceID
            'GetRawInputDeviceInfo(device.hDevice, DeviceInfoTypes.RIDI_DEVICEINFO, deviceInfo, deviceInfo.Length) ' allways empty
            'Debug.Print("device " & i & "    type=" & device.dwType & vbCrLf & vbCrLf)
            deviceName = deviceName.Substring(deviceName.IndexOf("{"))
            deviceName = deviceName.Substring(0, deviceName.IndexOf("}") + 1)
            result.Add(New GetMousesResult With {.Cod = device.hDevice, .Type = device.dwType, .Guid = deviceName})
        Next
        Marshal.FreeHGlobal(buffer)
        Return result
    End Function
    Public Class GetMousesResult
        Public Cod As Int32
        Public Type As DeviceUsage
        Public Guid As String
        Public Shadows Function ToString() As String
            Dim deviceSearcher As Management.ManagementObjectSearcher = New System.Management.ManagementObjectSearcher($"Select ClassGuid , Caption From Win32_PnPEntity")
            Dim deviceList As Management.ManagementObjectCollection = deviceSearcher.Get()
            For Each device In deviceList
                If device.GetPropertyValue("ClassGuid") Is Nothing OrElse Not CStr(device.GetPropertyValue("ClassGuid")).Contains(Guid) Then Continue For
                Return device.GetPropertyValue("Caption")
                'For Then Each p In device.Properties
                'If Not p.IsArray Then Debug.Print(p.Name & " = " & p.Value)
                'Debug.Print(device.GetPropertyValue("ClassGuid"))            =""     = "{745a17a0-74d3-11d0-b6fe-00a0c90f57da}"
                'Debug.Print(device.GetPropertyValue("DeviceID"))            'Debug.Print(device.GetPropertyValue("PNPDeviceID"))  = "{D6E55CA0-1A2E-4234-AAF3-3852170B492F}\VJOYRAWPDO\1&2D595CA7&0&VJOYINSTANCE00"    = "ROOT\HIDCLASS\0000"
                'Debug.Print(device.GetPropertyValue("Caption"))            'Debug.Print(device.GetPropertyValue("Name"))            'Debug.Print(device.GetPropertyValue("Description"))     = "VJoy Device"
                'Debug.Print("")
                'next
            Next device
            Return "?"
    ...
    <Flags()>
        ...
        If dwSize < 1 Then
            ...
            Return Nothing
        End If
        ' dwSize = 40 'Debug.Print(dwSize) ' checked thhat dwSize was allways 40
